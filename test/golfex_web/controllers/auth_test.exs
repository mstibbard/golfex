defmodule GolfexWeb.AuthTest do
  use GolfexWeb.ConnCase
  alias GolfexWeb.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(GolfexWeb.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Golfex.Accounts.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Golfex.Accounts.User{id: 123})
      |> send_resp(:ok, "")
  ...
  test "login with a password mismatch", %{conn: conn} do
    ...
    assert {:error, :unauthorized, _conn} = Auth.login_by_email_and_pass(conn, "me@test", "wrong")
  end
end
